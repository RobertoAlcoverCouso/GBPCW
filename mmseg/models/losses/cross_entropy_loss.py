...
def oh(lbl):
    lbl[lbl == 255] = 19
    return F.one_hot(lbl, 20).permute(0,3,1,2)[:,:-1,:,:]
...
def cross_entropy(pred,
    ...
    targets = oh(label.clone().detach())
    class_pixels = targets.transpose(0,1).flatten(start_dim=1).sum(1) + 1 
    output_d = pred.detach()
    p = F.softmax(output_d.requires_grad_(True), dim=1)
    loss_d = -torch.mul(targets, torch.log(p)).transpose(0,1).flatten(start_dim=1).sum(1)/class_pixels
    del p,class_pixels,targets
    #print(loss_d)
    ...
    try:
        class_weight = compute_weights(lossgrad, 1)
    except:
        class_weight = None
