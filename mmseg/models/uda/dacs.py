...
from mmseg.models.utils.dacs_transforms import (denorm, get_class_masks,
                                                get_mean_std, strong_transform, MMD_loss)
...
class DACS(UDADecorator):
    ...
    def feat_homogeneity(self, f1, f2, mask=None):
        #if mask is not None:
        #    return self.MMD(f1[mask], f2[mask])
        return self.MMD(f1,f2)
            ...
            for s in range(len(feat_imnet)):
                if self.fdist_classes is not None:
                    ...
                    fdist_mask = torch.any(gt_rescaled[..., None] == fdclasses,
                                           -1)
                    fd_s = self.feat_homogeneity(feat[s], feat_imnet[s],
                                                 fdist_mask)
                    feat_dist += fd_s
                    if fd_s != 0:
                        n_feat_nonzero += 1
